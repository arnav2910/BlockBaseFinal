...
function App() {
  ...
  const contractAddress = "0xeaDE0fB9Ab634530C5ffe50C3FD4E1d5f70A0619"; // Paste the address recieved from Remix IDE here
  ...
  useEffect(() => {
    ...
    const getUserName = async (contract, address) => {
    try {
      const [name] = await contract.getPerson(address);
      getUserName(name);
    } catch (error) {
  ...
  //To retrive the name if given address
  const getMyName = async () => {
  if (!contract || !account) return;
  try {
    const person = await contract.getPerson(account);
    alert(`Your registered name is: ${person.name}`);
  } catch (error) {
